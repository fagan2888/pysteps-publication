...
A localized version of STEPS called SSEPS (as in "short-space ensemble prediction
system") is verified for an increasingly small localization window.
"""
...
verification = {
    "experiment_name"   : "localization_2",
    "overwrite"         : True,            # to recompute nowcasts
    "v_thresholds"      : [0.1,2.,5.],     # [mm/h]
    "v_leadtimes"       : [60],            # [min]
    "v_accu"            : None,            # [min]
...
if experiment["vel_pert_method"] == "bps":
    ...
    for event in events:
        ...
        while startdate + datetime.timedelta(minutes = p["n_lead_times"]*ds.timestep) < enddate:
            ...
            if run_exist:
                ...
                R, _, metadata = stp.io.read_timeseries(input_files, importer,
                                    **ds.importer_kwargs)
                ...
                # Check imf
                R_ = R[-1, :, :]
                this_mm = np.nanmean(R_[R_ > 0])
                print("MM = %.3f mmh-1" % this_mm)

                # Check war
                this_war = (R[-1, :, :] > metadata["zerovalue"]).sum() / np.isfinite(R[-1, :, :]).sum()
                print("WAR = %.3f" % this_war)

                if this_war > p["min_war"] and this_mm > p["min_mm"] and not badobs:
                    ...
                    exporter = stp.io.initialize_forecast_exporter_netcdf(outfn, startdate,
                                      ...
                                      p["n_ens_members"], metadata0, incremental="timestep")

                    # start the nowcast
                    nwc = stp.nowcasts.sseps.forecast
                    nwc(R, metadata, UV, p["n_lead_times"], p["win_size"],
                            p["n_ens_members"], p["n_cascade_levels"],
                            num_workers=p["n_ens_members"],
            ...
            if badops_propr < .1:
                ...
                if os.path.isfile(infn):
                    ...
                    for i,lt in enumerate(p["v_leadtimes"]):
                        ...
                        # loop thresholds
                        for thr in p["v_thresholds"]:
                            P_fct = stp.postprocessing.ensemblestats.excprob(R_fct[:, idlt, :, :],
                                                                    thr, ignore_nan=True)
                            # reliability diagram
