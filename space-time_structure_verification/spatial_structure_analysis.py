...
# Experiment parameters (do not change order!)
...
for pei,pe in enumerate(precipevents):
    ...
    while curdate <= enddate:
        ...
        for nowcast in nowcast_types:
            ...
            if nowcast == "SPROG":
                ax.set_title('SPROG', fontsize=18)
            ax.set_ylim([-30,60])
            ...
            # Savefig
            figname = 'figures/' + data_source[0:3] + '_' + curdate.strftime("%Y%m%d%H%M") + '_' + nowcast + '_nlevels' + str(n_levels) + '_spectra.' + fmt
